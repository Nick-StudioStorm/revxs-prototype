...
import { AlertTriangle, Bell, BookOpen, Bot, Building2, CalendarClock, Check, Clock3, Database, FileText, Gauge, Inbox, LayoutList, MailCheck, MessageSquareReply, Pause, Play, Radar, Send, Settings, ShieldCheck, SkipForward, Sparkles, Target, UserRound, Users, X } from "lucide-react";
...
import type { Opportunity, Section } from "@/lib/types";
...
export default function RevxsApp({ initialSection }: { initialSection: Section }) {
  ...
  const [openTabs, setOpenTabs] = useState<string[]>([initialOpportunities[0].id]);
  const [metricFilter, setMetricFilter] = useState<MetricKey>("needsAction");
  const [draftMode, setDraftMode] = useState(false);
  const [onboarding, setOnboarding] = useState(0);
  const [statusNote, setStatusNote] = useState("System synced 3m ago");
  ...
  const metrics = useMemo(() => getMetrics(opportunities), [opportunities]);
  const filtered = useMemo(() => filterOpportunities(opportunities, metricFilter), [opportunities, metricFilter]);
  ...
  function openOpportunity(id: string) {
    ...
    setSection("queue");
    setDraftMode(false);
    if (onboarding === 2) setOnboarding(3);
  }

  function updateSelected(update: Partial<Opportunity>, note: string) {
    setOpportunities((current) => current.map((item) => (item.id === selected.id ? { ...item, ...update } : item)));
    setStatusNote(note);
  }
  ...
  return (
    <main className="flex min-h-screen bg-ink text-slate-100">
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <SystemBar metrics={metrics} active={metricFilter} statusNote={statusNote} onboarding={onboarding === 0} onSelect={(key) => { setMetricFilter(key); setSection("queue"); if (onboarding === 0) setOnboarding(1); }} />
        {section === "queue" ? (
          <div className="relative flex min-h-0 flex-1 flex-col">
            {onboarding < 3 && <Onboarding step={onboarding} onNext={() => setOnboarding((step) => Math.min(step + 1, 3))} onSkip={() => setOnboarding(3)} />}
            <Tabs tabs={openTabs} selected={selected} opportunities={opportunities} onSelect={setSelectedId} onClose={(id) => setOpenTabs((tabs) => tabs.filter((tab) => tab !== id).length ? tabs.filter((tab) => tab !== id) : [selected.id])} />
            <PanelGroup direction="horizontal" className="min-h-0 flex-1">
              <Panel defaultSize={40} minSize={30}>
                <QueuePanel opportunities={filtered} selected={selected} metricFilter={metricFilter} onboarding={onboarding} onOpen={openOpportunity} />
              </Panel>
              ...
              <Panel minSize={42}>
                <Workspace
                  ...
                  draftMode={draftMode}
                  onEdit={() => setDraftMode(true)}
                  onCancelEdit={() => setDraftMode(false)}
                  onDraft={(field, value) => updateSelected({ [field]: value } as Partial<Opportunity>, "Draft edited locally")}
                  onApprove={() => updateSelected({ stage: "Sent", status: "Waiting on prospect", waitingOn: "Prospect", leak: false, lastTouched: "Just now" }, "Approved and marked sent")}
                  onSchedule={() => updateSelected({ status: "Scheduled", nextFollowUp: "Tomorrow 09:30" }, "Follow-up scheduled")}
                  onBooked={() => updateSelected({ stage: "Call Booked", status: "Needs preparation", waitingOn: "You" }, "Meeting marked booked")}
                  onSkip={() => updateSelected({ status: "Skipped", leak: false, priority: "Low" }, "Opportunity skipped")}
                />
              </Panel>
            </PanelGroup>
          </div>
          ...
          <OutboundView campaigns={campaignState} setCampaigns={setCampaignState} onOpen={openOpportunity} onStatus={setStatusNote} />
        ) : (
    ...
    <aside className="hidden w-64 shrink-0 border-r border-line bg-[#080c12] lg:block">
      ...
      <nav className="space-y-1 p-3">
        {navItems.map((item) => {
          ...
          return <Link href={item.section === "queue" ? "/" : `/${item.section}`} key={item.section} onClick={() => onNavigate(item.section)} className={clsx("flex h-10 items-center gap-3 rounded-md px-3 text-sm text-slate-400 transition hover:bg-panelSoft hover:text-slate-100", active === item.section && "bg-panelSoft text-emerald-200 shadow-glow")}><IconComponent className="h-4 w-4" />{item.label}</Link>;
        })}
      </nav>
    </aside>
  );
...
function SystemBar({ metrics, active, statusNote, onboarding, onSelect }: { metrics: Record<MetricKey, number | string>; active: MetricKey; statusNote: string; onboarding: boolean; onSelect: (key: MetricKey) => void }) {
  const items: { key: MetricKey; label: string; icon: Icon }[] = [
  ...
  return (
    <header className={clsx("relative border-b border-line bg-[#070b11] px-4 py-3", onboarding && "ring-2 ring-emerald-300/70")}>
      <div className="mb-3 flex items-center justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.16em] text-slate-500">Signal {"->"} Action System</p><h1 className="text-lg font-semibold">Revenue System Bar</h1></div><div className="flex items-center gap-2 text-xs text-slate-400"><span className="h-2 w-2 rounded-full bg-emerald-300" />{statusNote}</div></div>
      <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-9">{items.map((item) => { const IconComponent = item.icon; return <button key={item.key} type="button" title={`Filter queue by ${item.label}`} onClick={() => onSelect(item.key)} className={clsx("min-h-20 rounded-md border border-line bg-panel px-3 py-2 text-left transition hover:border-emerald-300/50 hover:bg-panelSoft", active === item.key && "border-emerald-300/60 bg-emerald-400/10")}><div className="flex items-center justify-between gap-2"><IconComponent className="h-4 w-4 text-emerald-300" /><span className="text-lg font-semibold">{metrics[item.key]}</span></div><p className="mt-2 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-500">{item.label}</p></button>; })}</div>
    </header>
  );
}

function Tabs({ tabs, selected, opportunities, onSelect, onClose }: { tabs: string[]; selected: Opportunity; opportunities: Opportunity[]; onSelect: (id: string) => void; onClose: (id: string) => void }) {
  return <div className="flex h-12 shrink-0 items-center gap-2 overflow-x-auto border-b border-line bg-[#080c12] px-3">{tabs.map((id) => { const opportunity = opportunities.find((item) => item.id === id) ?? selected; return <button key={id} type="button" onClick={() => onSelect(id)} className={clsx("flex h-8 items-center gap-2 rounded-md border border-line px-3 text-xs text-slate-400", selected.id === id && "border-emerald-300/50 bg-panelSoft text-slate-100")}><span className="max-w-44 truncate">{opportunity.company}</span><X className="h-3.5 w-3.5" onClick={(event) => { event.stopPropagation(); onClose(id); }} /></button>; })}</div>;
}

function QueuePanel({ opportunities, selected, metricFilter, onboarding, onOpen }: { opportunities: Opportunity[]; selected: Opportunity; metricFilter: MetricKey; onboarding: number; onOpen: (id: string) => void }) {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden border-r border-line">
      <div className={clsx("border-b border-line p-4", onboarding === 2 && "ring-2 ring-emerald-300/70")}><h2 className="text-base font-semibold">Revenue Queue</h2><p className="text-xs text-slate-500">{filterLabel(metricFilter)} / {opportunities.length} visible</p></div>
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {groups.map((group) => { const items = opportunities.filter((item) => item.section === group || (group === "Revenue Leaks" && item.leak)); if (!items.length) return null; return <section key={group} className="mb-4"><div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{group}</h3><span className="text-xs text-slate-600">{items.length}</span></div><div className="space-y-2">{items.map((item) => <OpportunityCard key={`${group}-${item.id}`} opportunity={item} active={selected.id === item.id} onOpen={onOpen} />)}</div></section>; })}
      </div>
    </div>
  );
}
...
function filterOpportunities(opportunities: Opportunity[], filter: MetricKey) {
